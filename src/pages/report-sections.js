...
export function renderReportHero(data) {
  ...
  for (var i = 0; i < data.heroMetrics.length; i++) {
    ...
    metricsHtml +=
      '<div class="rh-metric"><div class="rh-metric-label">' +
      m.label +
      '</div><div class="' +
      cls +
      '">' +
  ...
  var sparklineHtml = data.priceHistory
    ? renderSparkline(data.priceHistory)
    : "";
  ...
  if (data.hero && data.hero.embedded_thesis) {
    embeddedThesisHtml =
      ...
      '<p class="rh-spec-text">' +
      data.hero.embedded_thesis +
      "</p>" +
      "</div>";
  }
  ...
  if (data.hero && data.hero.position_in_range) {
    ...
    var current = parseFloat(
      data._livePrice || data.price || pir.current_price,
    );
    var prices = worlds.map(function (w) {
    ...
    for (var i = 0; i < worlds.length; i++) {
      ...
      worldMarkersHtml +=
        '<div class="pir-world" style="left:' +
        pct +
        '%">' +
        '<div class="pir-world-tick"></div>' +
        '<div class="pir-world-price">A$' +
        w.price.toFixed(0) +
        "</div>" +
        '<div class="pir-world-label">' +
        w.label +
        "</div>" +
        "</div>";
    }
    ...
    positionInRangeHtml =
      ...
      '<div class="pir-bar">' +
      worldMarkersHtml +
      '<div class="pir-current" style="left:' +
      currentPct +
      '%">' +
      '<div class="pir-current-dot">&#9679;</div>' +
      '<div class="pir-current-label">A$' +
      current.toFixed(2) +
      "</div>" +
      "</div>" +
      "</div>" +
  ...
  if (
    data.hero &&
    data.hero.position_in_range &&
    data.hero.position_in_range.worlds &&
    ...
    var vrFairLow =
      (parseFloat(vrWorlds[1].price) + parseFloat(vrWorlds[2].price)) / 2;
    var vrFairHigh =
      (parseFloat(vrWorlds[2].price) + parseFloat(vrWorlds[3].price)) / 2;
    ...
    var vrCurrPct = Math.min(
      100,
      Math.max(0, ((vrCurrent - vrMin) / vrRange) * 100),
    ).toFixed(1);
    ...
    if (data.heroMetrics) {
      for (var mi = 0; mi < data.heroMetrics.length; mi++) {
        if (data.heroMetrics[mi].label === "FWD P/E") {
          vrPE = data.heroMetrics[mi].value;
          break;
        }
      }
    }
    ...
    valuationRangeHtml =
      ...
      '<div class="vr-bar">' +
      '<div class="vr-current" style="left:' +
      vrCurrPct +
      '%">A$' +
      vrCurrent.toFixed(2) +
      "</div>" +
      '<div class="vr-marker" style="left:' +
      vrBearPct +
      '%">' +
      '<div class="vr-marker-price">A$' +
      vrBear.toFixed(2) +
      "</div>" +
      ...
      '<div class="vr-marker-price" style="color:var(--accent-teal)">A$' +
      vrFairLow.toFixed(2) +
      " &ndash; A$" +
      vrFairHigh.toFixed(2) +
      "</div>" +
  ...
  if (data.hero && data.hero.skew) {
    var skewCls =
      data.hero.skew === "DOWNSIDE"
        ? "rh-skew-down"
        : data.hero.skew === "UPSIDE"
          ? "rh-skew-up"
          : "rh-skew-balanced";
    skewIndicatorHtml =
      '<div class="rh-spec-block rh-skew-indicator">' +
      '<span class="rh-spec-label">SKEW: </span>' +
      '<span class="rh-skew-value ' +
      skewCls +
      '">' +
      data.hero.skew +
      "</span>" +
      (data.hero.skew_description
        ? '<p class="rh-spec-text">' + data.hero.skew_description + "</p>"
        : "") +
      "</div>";
  }
  ...
  return (
    ...
    data.sector +
    "</div>" +
    '<div class="rh-sector-tag">' +
    data.heroDescription +
    "</div>" +
    (data.heroCompanyDescription
      ? '<div class="rh-company-desc">' + data.heroCompanyDescription + "</div>"
      : "") +
    ...
    "</button>" +
    '<span class="refresh-timestamp" id="refresh-ts-' +
    data.ticker +
    '">' +
    ...
    data.ticker +
    '" style="display:none">' +
    '<div class="progress-bar"><div class="progress-fill" id="refresh-fill-' +
    data.ticker +
    '"></div></div>' +
    '<span class="progress-label" id="refresh-label-' +
    data.ticker +
    '">Searching for new data...</span>' +
    "</div>" +
    "</div>" +
    "</div>" +
    '<div class="rh-right">' +
    sparklineHtml +
    '<div class="rh-price"><span class="rh-price-currency">' +
    data.currency +
    "</span>" +
    data.price +
    "</div>" +
  ...
  var arrow =
    dir === "downside"
      ...
      : dir === "upside"
        ...
        : "&#9670; BALANCED";
  var scoreCls =
    skew.score > 5 ? "positive" : skew.score < -5 ? "negative" : "neutral";
  var scoreLabel = (skew.score > 0 ? "+" : "") + skew.score;

  return (
    '<div class="risk-skew-bar">' +
    '<div class="rsb-inner">' +
    '<span class="rsb-label">Thesis Skew</span>' +
    '<span class="skew-badge ' +
    dir +
    '">' +
    arrow +
    "</span>" +
    '<div class="skew-bar-track" style="width:80px;height:10px;margin:0 4px">' +
    '<div class="skew-bar-bull" style="width:' +
    skew.bull +
    '%"></div>' +
    '<div class="skew-bar-bear" style="width:' +
    skew.bear +
    '%"></div>' +
    "</div>" +
    '<span class="skew-score ' +
    scoreCls +
    '" style="font-size:0.82rem">' +
    scoreLabel +
    "</span>" +
    '<span class="rsb-rationale">' +
    data.skew.rationale +
    "</span>" +
    "</div>" +
    "</div>"
...
export function renderVerdict(data) {
  var v = data.verdict;
  var borderStyle = v.borderColor
    ? ' style="border-color: ' + v.borderColor + '"'
    : "";
  ...
  for (var i = 0; i < v.scores.length; i++) {
    ...
    var dirAttr = data.hypotheses[i]
      ? ' data-dir="' + (data.hypotheses[i].dirClass || "dir-neutral") + '"'
      : "";
    scoresHtml +=
      '<div class="vs-item"' +
      dirAttr +
      ...
      s.label +
      "</div>" +
      '<div class="vs-score" style="color:' +
      s.scoreColor +
      '">' +
      norm[i] +
      "%</div>" +
      '<div class="vs-direction"' +
      dirStyle +
      ">" +
      s.dirArrow +
      " " +
...
export function renderSectionNav(data) {
  ...
  if (!data.technicalAnalysis) {
    sections = sections.filter(function (s) {
      return s[0] !== "technical";
    });
  }
  if (!data.priceDrivers) {
    sections = sections.filter(function (s) {
      return s[0] !== "price-drivers";
    });
  }
  if (data.goldAnalysis) {
    // Insert Gold Analysis before Research Chat
    var chatIdx = sections.findIndex(function (s) {
      return s[0] === "chat";
    });
    if (chatIdx >= 0)
      sections.splice(chatIdx, 0, ["gold-analysis", "Gold Analysis"]);
  }
...
export function renderIdentity(data) {
  ...
  return (
    ...
    "</tbody>" +
    "</table>" +
    '<p class="rs-text"><strong>Business overview:</strong> ' +
    id.overview +
    "</p>" +
    "</div></div>"
  );
...
export function renderHypotheses(data) {
  ...
  for (var i = 0; i < data.hypotheses.length; i++) {
    ...
    if (h.requires && h.requires.length > 0) {
      requiresHtml =
        '<div class="hc-subtitle">Requires</div><ul class="hc-list requires">';
    ...
    if (h.contradicting && h.contradicting.length > 0) {
      contradictHtml =
        '<div class="hc-subtitle">' +
        h.contradictingLabel +
        '</div><ul class="hc-list contradicts">';
    ...
    cardsHtml +=
      ...
      dominantCls +
      '">' +
      '<div class="hc-header"><div class="hc-title">' +
      h.title +
      '</div><div class="hc-status ' +
      h.statusClass +
      '">' +
      h.statusText +
      "</div></div>" +
      '<div class="hc-score-row"><div class="hc-score-number">' +
      normScore +
      '</div><div class="hc-score-bar"><div class="hc-score-fill" style="width:' +
      normWidth +
      '"></div></div><div class="hc-score-meta">' +
      h.scoreMeta +
      "</div></div>" +
      '<p class="hc-desc">' +
      h.description +
...
export function renderNarrative(data) {
  ...
  return (
    ...
    n.priceImplication.label +
    "</div>" +
    "<p>" +
    n.priceImplication.content +
    "</p>" +
    "</div>" +
...
export function renderEvidenceCard(card) {
  ...
  if (card.table) {
    ...
    tableHtml =
      '<table class="identity-table" style="margin-bottom:var(--space-md)">' +
      "<thead><tr>" +
      thHtml +
      "</tr></thead>" +
      "<tbody>" +
...
export function renderAlignmentSummary(data) {
  ...
  for (var i = 0; i < as.rows.length; i++) {
    ...
    var cellFn = function (cell) {
      ...
      return (
        '<td class="' + cell.class + '"' + styleAttr + ">" + cell.text + "</td>"
      );
    };
  ...
  tbHtml +=
    ...
    "</td>" +
    '<td style="font-family:var(--font-data)' +
    (sum.n2Color ? ";color:" + sum.n2Color : "") +
    '">' +
    sum.n2 +
    "</td>" +
    '<td style="font-family:var(--font-data)' +
    (sum.n3Color ? ";color:" + sum.n3Color : "") +
    '">' +
    sum.n3 +
    "</td>" +
    '<td style="font-family:var(--font-data)">' +
  ...
  return (
    '<div class="rs-subtitle">Evidence Alignment Summary</div>' +
...
export function renderTripwires(data) {
  ...
  for (var i = 0; i < tw.cards.length; i++) {
    ...
    for (var c = 0; c < card.conditions.length; c++) {
      ...
      conditionsHtml +=
        '<div class="tw-condition">' +
        '<div class="tw-cond-if ' +
        cond.valence +
        '">' +
        cond.if +
        "</div>" +
        '<div class="tw-cond-then">' +
        cond.then +
        "</div>" +
        "</div>";
    }
    ...
    cardsHtml +=
      ...
      resolvedCls +
      '">' +
      '<div class="tw-header"><div class="tw-date">' +
      card.date +
      '</div><div class="tw-name">' +
      card.name +
      "</div></div>" +
      '<div class="tw-conditions">' +
      conditionsHtml +
      "</div>" +
      '<div class="tw-source">' +
      (card.source || "") +
...
export function renderGaps(data) {
  ...
  for (var i = 0; i < g.coverageRows.length; i++) {
    ...
    var confClass = r.confidenceClass
      ? ' class="' + r.confidenceClass + '"'
      : "";
    coverageHtml +=
      "<tr>" +
      "<td>" +
  ...
  for (var j = 0; j < g.couldntAssess.length; j++) {
    calloutsHtml +=
      '<div class="callout"><p>' + g.couldntAssess[j] + "</p></div>";
  }
...
export function renderTAChart(data) {
  ...
  var chartTitle = useLive
    ? (n > 500 ? "3" : n > 250 ? "2" : "1") +
      "-Year Daily Price &amp; Moving Averages"
    : "12-Month Daily Price &amp; Moving Averages";
  var liveLabel = useLive
    ? '<span class="ta-chart-live-badge">LIVE</span>'
    : '<span class="ta-chart-static-badge">STATIC</span>';
  ...
  function xPos(idx) {
    return padL + (idx / (n - 1)) * cW;
  }
  function yPos(val) {
    return padT + (1 - (val - pMin) / pRange) * cH;
  }

  var supportPrice = ta ? parseFloat(ta.keyLevels.support.price) || null : null;
  var resistPrice = ta
    ? parseFloat(ta.keyLevels.resistance.price) || null
    : null;
  var curPrice =
    parseFloat(useLive && live.currentPrice ? live.currentPrice : data.price) ||
    0;
  var cur = data.currency;

  var svg =
    '<svg class="ta-chart-svg" viewBox="0 0 ' +
    W +
    " " +
    H +
  ...
  for (var s = 0; s < niceSteps.length; s++) {
    if (niceSteps[s] * magnitude >= gridStep) {
      bestStep = niceSteps[s] * magnitude;
      break;
    }
  }
  for (
    var gv = Math.ceil(pMin / bestStep) * bestStep;
    gv <= pMax;
    gv += bestStep
  ) {
    var gy = yPos(gv);
  ...
  if (supportPrice && supportPrice >= pMin && supportPrice <= pMax) {
    ...
    svg +=
      ...
      '" y2="' +
      sy.toFixed(1) +
      '" stroke="#3DAA6D" stroke-width="0.8" stroke-dasharray="6,4" opacity="0.6"/>';
    svg +=
      '<text x="' +
      (padL + 4) +
      '" y="' +
      (sy - 4).toFixed(1) +
      '" fill="#3DAA6D" font-family="JetBrains Mono, monospace" font-size="7.5" opacity="0.8">S ' +
      cur +
  ...
  if (resistPrice && resistPrice >= pMin && resistPrice <= pMax) {
    ...
    svg +=
      ...
      '" y2="' +
      ry.toFixed(1) +
      '" stroke="#D45555" stroke-width="0.8" stroke-dasharray="6,4" opacity="0.6"/>';
    svg +=
      '<text x="' +
      (padL + 4) +
      '" y="' +
      (ry - 4).toFixed(1) +
      '" fill="#D45555" font-family="JetBrains Mono, monospace" font-size="7.5" opacity="0.8">R ' +
      cur +
      resistPrice.toFixed(2) +
      "</text>";
  }
  ...
  if (highs && lows) {
    ...
    for (var i = 0; i < n; i++) {
      ...
      hlUpper +=
        (hlUpper === "" ? "M" : "L") + x + "," + yPos(highs[i]).toFixed(1);
      hlLower =
        x +
        "," +
        yPos(lows[i]).toFixed(1) +
        (hlLower === "" ? "" : "L" + hlLower);
    }
    if (hlUpper && hlLower) {
      svg +=
  ...
  for (var i = 0; i < n; i++) {
    ...
    ma200Path +=
      ...
      xPos(i).toFixed(1) +
      "," +
      yPos(ma200Arr[i]).toFixed(1);
  }
  if (ma200Path)
    svg +=
      '<path d="' +
      ma200Path +
      '" fill="none" stroke="#4A8ECC" stroke-width="1.3" opacity="0.8"/>';
  ...
  for (var i = 0; i < n; i++) {
    ...
    ma50Path +=
      ...
      xPos(i).toFixed(1) +
      "," +
      yPos(ma50Arr[i]).toFixed(1);
  }
  if (ma50Path)
    svg +=
      '<path d="' +
      ma50Path +
      '" fill="none" stroke="#D4A03C" stroke-width="1.3" opacity="0.8"/>';

  var pricePath = "M" + xPos(0).toFixed(1) + "," + yPos(closes[0]).toFixed(1);
  for (var i = 1; i < n; i++)
    pricePath += "L" + xPos(i).toFixed(1) + "," + yPos(closes[i]).toFixed(1);
  var areaPath =
    pricePath +
    "L" +
    xPos(n - 1).toFixed(1) +
    ...
    "Z";
  svg +=
    '<defs><linearGradient id="priceGrad-' +
    data.ticker +
    '" x1="0" y1="0" x2="0" y2="1">';
  svg +=
  ...
  svg +=
    '<path d="' + areaPath + '" fill="url(#priceGrad-' + data.ticker + ')"/>';
  ...
  if (useLive) {
    ...
    for (var m = 0; m <= 11; m++) {
      ...
      if (labelDate.getMonth() === 0)
        label += " '" + String(labelDate.getFullYear()).slice(2);
      var lx = xPos(idx);
      if (lx < padL + 15 || lx > padL + cW - 15) continue;
      svg +=
        '<text x="' +
  ...
  svg +=
    ...
    '" y="17" fill="' +
    C.legendText +
    '" font-family="Inter, sans-serif" font-size="8">Price</text>';
  if (highs) {
    svg +=
      '<rect x="' +
      (lx0 + 44) +
      '" y="10" width="16" height="8" rx="1" fill="' +
      C.hlBand +
      '" opacity="0.12"/>';
    svg +=
      '<text x="' +
      ...
      '" y="17" fill="' +
      C.legendText +
      '" font-family="Inter, sans-serif" font-size="8">High/Low</text>';
    lx0 += 56;
  }
  svg +=
    '<line x1="' +
    ...
    '" y1="14" x2="' +
    (lx0 + 68) +
    '" y2="14" stroke="#D4A03C" stroke-width="1.5"/>';
  svg +=
    '<text x="' +
    (lx0 + 72) +
    '" y="17" fill="' +
    C.legendText +
    '" font-family="Inter, sans-serif" font-size="8">50d MA</text>';
  svg +=
    '<line x1="' +
    (lx0 + 112) +
    '" y1="14" x2="' +
    (lx0 + 128) +
    '" y2="14" stroke="#4A8ECC" stroke-width="1.5"/>';
  svg +=
    '<text x="' +
    (lx0 + 132) +
    '" y="17" fill="' +
    C.legendText +
    '" font-family="Inter, sans-serif" font-size="8">200d MA</text>';
  svg +=
    '<line x1="' +
    (lx0 + 182) +
    '" y1="14" x2="' +
    (lx0 + 198) +
    '" y2="14" stroke="#3DAA6D" stroke-width="1" stroke-dasharray="4,3"/>';
  svg +=
    '<text x="' +
    (lx0 + 202) +
    '" y="17" fill="' +
    C.legendText +
    '" font-family="Inter, sans-serif" font-size="8">Support</text>';
  svg +=
    '<line x1="' +
    (lx0 + 242) +
    '" y1="14" x2="' +
    (lx0 + 258) +
    '" y2="14" stroke="#D45555" stroke-width="1" stroke-dasharray="4,3"/>';
  svg +=
    '<text x="' +
    (lx0 + 262) +
    '" y="17" fill="' +
    C.legendText +
    '" font-family="Inter, sans-serif" font-size="8">Resistance</text>';

  svg += "</svg>";

  return (
    '<div class="ta-chart-container">' +
    '<div class="ta-chart-header"><div class="ta-chart-title">' +
    chartTitle +
    "</div>" +
    liveLabel +
    "</div>" +
...
export function renderTechnicalAnalysis(data) {
  ...
  var regimeHtml =
    '<div class="ta-regime-bar">' +
    '<div class="ta-regime-item"><div class="ta-regime-label">Regime</div><div class="ta-regime-value">' +
    ta.regime +
    "</div></div>" +
    '<div class="ta-regime-item"><div class="ta-regime-label">Clarity</div><div class="ta-regime-value">' +
    ta.clarity +
    "</div></div>" +
    '<div class="ta-regime-item"><div class="ta-regime-label">Trend</div><div class="ta-regime-value ' +
    (/down/i.test(ta.trend.direction)
      ? "ta-down"
      : /up|recover/i.test(ta.trend.direction)
        ? "ta-up"
        : "") +
    '">' +
    ta.trend.direction +
    ...
    ta.trend.duration +
    ")</div></div>" +
    '<div class="ta-regime-item"><div class="ta-regime-label">Structure</div><div class="ta-regime-value">' +
    ta.trend.structure +
    "</div></div>" +
    '<div class="ta-regime-item"><div class="ta-regime-label">Support</div><div class="ta-regime-value">' +
    ta.price.currency +
    ta.keyLevels.support.price.toFixed(2) +
    "</div></div>" +
    '<div class="ta-regime-item"><div class="ta-regime-label">Resistance</div><div class="ta-regime-value">' +
    ta.price.currency +
    ta.keyLevels.resistance.price.toFixed(2) +
    "</div></div>" +
    "</div>";

  var maHtml =
    '<div class="rs-subtitle">Moving Averages</div>' +
    ...
    "<td>" +
    ta.price.currency +
    ta.movingAverages.ma50.value.toFixed(2) +
    "</td>" +
    '<td style="color:' +
    (ta.movingAverages.priceVsMa50 >= 0
      ? "var(--signal-green)"
      : "var(--signal-red)") +
    '">' +
    (ta.movingAverages.priceVsMa50 >= 0 ? "+" : "") +
    ta.movingAverages.priceVsMa50.toFixed(1) +
    "%</td>" +
    "<td>As at " +
    ta.movingAverages.ma50.date +
    "</td>" +
    "</tr>" +
    "<tr>" +
    ...
    "<td>" +
    ta.price.currency +
    ta.movingAverages.ma200.value.toFixed(2) +
    "</td>" +
    '<td style="color:' +
    (ta.movingAverages.priceVsMa200 >= 0
      ? "var(--signal-green)"
      : "var(--signal-red)") +
    '">' +
    (ta.movingAverages.priceVsMa200 >= 0 ? "+" : "") +
    ta.movingAverages.priceVsMa200.toFixed(1) +
    "%</td>" +
    "<td>As at " +
    ta.movingAverages.ma200.date +
    "</td>" +
    "</tr>" +
    "</tbody></table>";
  ...
  if (ta.movingAverages.crossover) {
    ...
    crossoverHtml =
      '<div class="ta-crossover-callout">' +
      '<div class="ta-crossover-label">' +
      cx.type +
      "</div>" +
      '<div class="ta-crossover-text">' +
      cx.description +
      " &mdash; " +
      cx.date +
      "</div>" +
  ...
  if (ta.inflectionPoints && ta.inflectionPoints.length > 0) {
    ...
    for (var i = 0; i < ta.inflectionPoints.length; i++) {
      ...
      inflRows +=
        "<tr>" +
        '<td class="ta-date-cell">' +
        ip.date +
        "</td>" +
        '<td class="ta-price-cell">' +
        ta.price.currency +
        ip.price.toFixed(2) +
        "</td>" +
        '<td class="ta-event-cell">' +
        ip.event +
        "</td>" +
        "</tr>";
    }
    inflHtml =
      '<div class="rs-subtitle">Price Inflection Points</div>' +
  ...
  var volHtml =
    ...
    '<div class="ta-metric-row"><div class="ta-metric-name">Latest vs 20-day avg</div><div class="ta-metric-val">' +
    ta.volume.latestVs20DayAvg.toFixed(1) +
    "x</div></div>" +
    '<div class="ta-metric-row"><div class="ta-metric-name">Date</div><div class="ta-metric-val">' +
    ta.volume.latestDate +
    "</div></div>";
  if (ta.volume.priorSpikes) {
    for (var v = 0; v < ta.volume.priorSpikes.length; v++) {
      var sp = ta.volume.priorSpikes[v];
      volHtml +=
        '<div class="ta-metric-row"><div class="ta-metric-name">' +
        sp.period +
        '</div><div class="ta-metric-val">' +
        sp.ratio.toFixed(1) +
        "x &mdash; " +
        sp.context +
        "</div></div>";
  ...
  volHtml +=
    ...
    '<div class="ta-metric-row"><div class="ta-metric-name">Latest daily range</div><div class="ta-metric-val">' +
    ta.volatility.latestRangePercent.toFixed(1) +
    "%</div></div>" +
    '<div class="ta-metric-row"><div class="ta-metric-name">30-day avg range</div><div class="ta-metric-val">' +
    ta.volatility.avgDailyRangePercent30.toFixed(1) +
    "%</div></div>" +
    '<div class="ta-metric-row"><div class="ta-metric-name">90-day avg range</div><div class="ta-metric-val">' +
    ta.volatility.avgDailyRangePercent90.toFixed(1) +
    "%</div></div>" +
    '<div class="ta-metric-row"><div class="ta-metric-name">Latest session</div><div class="ta-metric-val">' +
    ta.price.currency +
    ta.volatility.latestDailyRange.high.toFixed(2) +
    " &ndash; " +
    ta.price.currency +
    ta.volatility.latestDailyRange.low.toFixed(2) +
    "</div></div>" +
    "</div>" +
    "</div>";
  ...
  var pricePct =
    rangeSpan > 0 ? ((ta.price.current - mr.rangeLow) / rangeSpan) * 100 : 50;
  var ma50Pct =
    rangeSpan > 0
      ? ((ta.movingAverages.ma50.value - mr.rangeLow) / rangeSpan) * 100
      : 50;
  var ma200Pct =
    rangeSpan > 0
      ? ((ta.movingAverages.ma200.value - mr.rangeLow) / rangeSpan) * 100
      : 50;

  var mrHtml =
    '<div class="ta-mr-container">' +
    '<div class="ta-mr-title">Mean Reversion Positioning</div>' +
    '<div class="ta-mr-bar-track">' +
    '<div class="ta-mr-ma200-marker" style="left:' +
    ma200Pct.toFixed(1) +
    '%"></div>' +
    '<div class="ta-mr-ma50-marker" style="left:' +
    ma50Pct.toFixed(1) +
    '%"></div>' +
    '<div class="ta-mr-marker" style="left:' +
    pricePct.toFixed(1) +
    '%"></div>' +
    "</div>" +
    ...
    "<span>" +
    ta.price.currency +
    mr.rangeLow.toFixed(2) +
    "</span>" +
    "<span>" +
    ta.price.currency +
    mr.rangeHigh.toFixed(2) +
    "</span>" +
    "</div>" +
    '<div class="ta-mr-legend">' +
    '<div class="ta-mr-legend-item"><div class="ta-mr-legend-dot" style="background:var(--signal-red)"></div>Price (' +
    ta.price.currency +
    ta.price.current.toFixed(2) +
    ")</div>" +
    '<div class="ta-mr-legend-item"><div class="ta-mr-legend-dot" style="background:var(--signal-amber)"></div>50-Day MA (' +
    ta.price.currency +
    ta.movingAverages.ma50.value.toFixed(2) +
    ")</div>" +
    '<div class="ta-mr-legend-item"><div class="ta-mr-legend-dot" style="background:var(--signal-blue)"></div>200-Day MA (' +
    ta.price.currency +
    ta.movingAverages.ma200.value.toFixed(2) +
    ")</div>" +
    "</div>" +
    '<table class="ta-ma-table" style="margin-top:var(--space-sm)"><thead><tr><th>Measure</th><th>Value</th></tr></thead><tbody>' +
    '<tr><td class="ta-label-cell">vs 50-Day MA</td><td style="color:var(--signal-red)">' +
    mr.vsMa50.toFixed(1) +
    "%</td></tr>" +
    '<tr><td class="ta-label-cell">vs 200-Day MA</td><td style="color:var(--signal-red)">' +
    mr.vsMa200.toFixed(1) +
    "%</td></tr>" +
    '<tr><td class="ta-label-cell">12-Month Range Position</td><td>' +
    (mr.rangePosition <= 50 ? "Lower " : "Upper ") +
    mr.rangePosition +
    "%</td></tr>" +
    "</tbody></table>" +
    "</div>";
  ...
  if (ta.relativePerformance) {
    ...
    relHtml =
      '<div class="rs-subtitle">Relative Performance (' +
      rp.vsIndex.period +
      ")</div>" +
      ...
      "</tr></thead><tbody>" +
      "<tr>" +
      '<td style="font-family:var(--font-ui);font-weight:600;color:var(--text-primary)">' +
      rp.vsIndex.name +
      "</td>" +
      '<td style="color:' +
      (rp.vsIndex.stockReturn >= 0
        ? "var(--signal-green)"
        : "var(--signal-red)") +
      '">' +
      (rp.vsIndex.stockReturn >= 0 ? "+" : "") +
      rp.vsIndex.stockReturn.toFixed(1) +
      "%</td>" +
      '<td style="color:' +
      (rp.vsIndex.indexReturn >= 0
        ? "var(--signal-green)"
        : "var(--signal-red)") +
      '">' +
      (rp.vsIndex.indexReturn >= 0 ? "+" : "") +
      rp.vsIndex.indexReturn.toFixed(1) +
      "%</td>" +
      '<td style="color:' +
      (rp.vsIndex.relativeReturn >= 0
        ? "var(--signal-green)"
        : "var(--signal-red)") +
      '">' +
      (rp.vsIndex.relativeReturn >= 0 ? "+" : "") +
      rp.vsIndex.relativeReturn.toFixed(1) +
      "%</td>" +
      "</tr>" +
      "<tr>" +
      '<td style="font-family:var(--font-ui);font-weight:600;color:var(--text-primary)">' +
      ...
      "</td>" +
      '<td style="color:' +
      (rp.vsSector.stockReturn >= 0
        ? "var(--signal-green)"
        : "var(--signal-red)") +
      '">' +
      (rp.vsSector.stockReturn >= 0 ? "+" : "") +
      rp.vsSector.stockReturn.toFixed(1) +
      "%</td>" +
      '<td style="color:' +
      (rp.vsSector.sectorReturn >= 0
        ? "var(--signal-green)"
        : "var(--signal-red)") +
      '">' +
      (rp.vsSector.sectorReturn >= 0 ? "+" : "") +
      rp.vsSector.sectorReturn.toFixed(1) +
      "%</td>" +
      '<td style="color:' +
      (rp.vsSector.relativeReturn >= 0
        ? "var(--signal-green)"
        : "var(--signal-red)") +
      '">' +
      (rp.vsSector.relativeReturn >= 0 ? "+" : "") +
      rp.vsSector.relativeReturn.toFixed(1) +
      "%</td>" +
      "</tr>" +
      "</tbody></table>";
  }
  ...
  var levelsHtml =
    '<div class="rs-subtitle">Key Levels</div>' +
    '<table class="ta-ma-table"><thead><tr><th>Level</th><th>Price</th><th>Derivation</th></tr></thead><tbody>' +
    '<tr><td class="ta-label-cell">Support</td><td>' +
    ta.price.currency +
    ta.keyLevels.support.price.toFixed(2) +
    '</td><td style="font-family:var(--font-ui)">' +
    ta.keyLevels.support.method +
    "</td></tr>" +
    '<tr><td class="ta-label-cell">Resistance</td><td>' +
    ta.price.currency +
    ta.keyLevels.resistance.price.toFixed(2) +
    '</td><td style="font-family:var(--font-ui)">' +
    ta.keyLevels.resistance.method +
    "</td></tr>" +
    '<tr><td class="ta-label-cell">52-Week High</td><td>' +
    ta.price.currency +
    ta.keyLevels.fiftyTwoWeekHigh.price.toFixed(2) +
    '</td><td style="font-family:var(--font-ui)">' +
    ta.keyLevels.fiftyTwoWeekHigh.date +
    "</td></tr>" +
    '<tr><td class="ta-label-cell">52-Week Low</td><td>' +
    ta.price.currency +
    ta.keyLevels.fiftyTwoWeekLow.price.toFixed(2) +
    '</td><td style="font-family:var(--font-ui)">' +
    ta.keyLevels.fiftyTwoWeekLow.date +
    "</td></tr>" +
    "</tbody></table>";

  var footerHtml =
    '<div style="font-size:0.6rem;color:var(--text-muted);margin-top:var(--space-md);padding-top:var(--space-sm);border-top:1px solid var(--border)">' +
    "Analysis period: " +
    ta.period +
    " &bull; Generated: " +
    ta.date +
    " &bull; Source: " +
    (ta.source || "Continuum Technical Intelligence") +
    "</div>";

  return (
    '<div class="report-section ta-section" id="' +
    t +
    '-technical">' +
...
export function renderReportFooter(data) {
  return (
    ...
    '<div class="rf-meta-item">ID: ' +
    data.reportId +
    "</div>" +
    '<div class="rf-meta-item">Mode: Narrative Intelligence</div>' +
    '<div class="rf-meta-item">Domains: ' +
    data.footer.domainCount +
    "</div>" +
    '<div class="rf-meta-item">Hypotheses: ' +
    data.footer.hypothesesCount +
    "</div>" +
...
export function renderPDFDownload(data) {
  ...
  return (
    ...
    '<div class="report-download-subtitle">' +
    data.company +
    " (" +
    data.ticker +
    ".AX) &mdash; " +
    ...
    '<button class="btn-pdf-download institutional" onclick="generatePDFReport(\'' +
    t +
    "', 'institutional')\">" +
    ...
    "</button>" +
    '<button class="btn-pdf-download retail" onclick="generatePDFReport(\'' +
    t +
    "', 'retail')\">" +
...
export function renderHypSidebar(data) {
  ...
  var macCls =
    macSig > 10 ? "dir-up" : macSig < -10 ? "dir-down" : "dir-neutral";
  var secCls =
    secSig > 10 ? "dir-up" : secSig < -10 ? "dir-down" : "dir-neutral";

  var ref =
    typeof REFERENCE_DATA !== "undefined" ? REFERENCE_DATA[ticker] : null;
  var peValue = "\u2014";
  var revGrowthValue = "\u2014";
  if (data.heroMetrics) {
    for (var mi = 0; mi < data.heroMetrics.length; mi++) {
  ...
  if (ph && ph.length >= 2) {
    changePct =
      ((ph[ph.length - 1] - ph[ph.length - 2]) / ph[ph.length - 2]) * 100;
  ...
  inner +=
    '<div class="hs-stock-id">' +
    '<div class="hs-stock-ticker">' +
    (data.tickerFull || data.ticker || ticker) +
    "</div>" +
  ...
  inner +=
    '<div class="hs-overall-skew">' +
    '<span class="hs-skew-dir ' +
    skewDir +
    '">' +
    skewLabel +
    "</span>" +
    '<span class="hs-skew-score ' +
    skewDir +
    '">' +
    skewScoreStr +
    "</span>" +
  ...
  inner +=
    '<div class="hs-subhead">Ext. Environment</div>' +
    '<div class="hs-env-row">' +
    '<div class="hs-dot ' +
    macCls +
    '"></div>' +
    '<span class="hs-env-label">Macro</span>' +
    '<span class="hs-env-score">' +
    (macSig > 0 ? "+" : "") +
    macSig +
    "</span>" +
    "</div>" +
  ...
  if (vr && livePrice > 0) {
    ...
    var vrCurrPct = Math.min(
      100,
      Math.max(0, ((livePrice - vrBear) / vrRange) * 100),
    ).toFixed(1);
    inner +=
      '<div class="hs-section-head">Valuation Range</div>' +
      '<div class="hs-val-section">' +
      '<div class="hs-val-header">' +
      '<span class="hs-val-zone ' +
      vrZoneCls +
      '">' +
      vrZone +
      "</span>" +
      ...
      '<div class="hs-val-marker" style="left:' +
      vrCurrPct +
      '%"></div>' +
      "</div>" +
...
export function prepareHypotheses(data) {
  ...
  var colorMap = {
    "dir-up": "var(--signal-green)",
    "dir-down": "var(--signal-red)",
    "dir-neutral": "var(--signal-amber)",
  };
  ...
  if (data.verdict && data.verdict.scores) {
    ...
    for (var i = 0; i < hyps.length; i++) {
      ...
      if (matched) {
        matched.label =
          "N" + (i + 1) + " " + matched.label.replace(/^N\d+\s*/, "");
      ...
      for (var i = 0; i < hyps.length; i++) {
        newHeaders.push(
          "N" +
            (i + 1) +
            " " +
            hyps[i].title.replace(/^N\d+:\s*/, "").substring(0, 15),
        );
      }
  ...
  if (needsRemap) {
    ...
    if (data.discriminators) {
      if (data.discriminators.intro)
        data.discriminators.intro = remapN(data.discriminators.intro);
      if (data.discriminators.nonDiscriminating)
        data.discriminators.nonDiscriminating = remapN(
          data.discriminators.nonDiscriminating,
        );
      if (data.discriminators.rows) {
        for (var i = 0; i < data.discriminators.rows.length; i++) {
          var dRow = data.discriminators.rows[i];
          if (dRow.discriminatesBetween)
            dRow.discriminatesBetween = remapN(dRow.discriminatesBetween);
          if (dRow.currentReading)
            dRow.currentReading = remapN(dRow.currentReading);
    ...
    if (data.tripwires) {
      if (data.tripwires.intro)
        data.tripwires.intro = remapN(data.tripwires.intro);
      ...
      for (var i = 0; i < data.evidence.cards.length; i++) {
        ...
        if (evCard.tags) {
          for (var t = 0; t < evCard.tags.length; t++) {
            if (evCard.tags[t].text)
              evCard.tags[t].text = remapN(evCard.tags[t].text);
          }
        }
    ...
    if (data.verdict && data.verdict.text)
      data.verdict.text = remapN(data.verdict.text);
    if (data.skew && data.skew.rationale)
      data.skew.rationale = remapN(data.skew.rationale);
  }
}
...
export function renderOvercorrectionBanner(data) {
  ...
  var label =
    oc.reviewResult && oc.reviewResult.confirmed
      ? "&#10004; Overcorrection Confirmed"
      : "&#9888; Possible Overcorrection Detected";
  var message =
    oc.message || "Price move exceeded threshold  --  scores under review.";
  var reviewHtml = oc.reviewDate
    ? '<div class="oc-review">5-day review scheduled: ' +
      oc.reviewDate +
      "</div>"
    : "";
  if (oc.reviewResult) {
  ...
  return (
    '<div class="overcorrection-banner' +
    cls +
    '">' +
    '<div class="oc-label">' +
    label +
...
export function renderNarrativeTimeline(data) {
  var t = data.ticker.toLowerCase();
  return (
    '<div class="report-section narrative-timeline-section" id="' +
    t +
    '-narrative-timeline">' +
    ...
    "</div>" +
    '<div class="nt-chart-canvas-wrap" id="nt-wrap-' +
    data.ticker +
    '">' +
    '<div class="nt-chart-loading" id="nt-loading-' +
    data.ticker +
    '">' +
    ...
    "</div>" +
    '<div class="nt-flip-legend" id="nt-legend-' +
    data.ticker +
    '"></div>' +
    "</div>" +
    "</div>" +
...
export function initNarrativeTimelineChart(ticker) {
  ...
  loadNarrativeHistory(ticker, function (histData) {
    ...
    if (!histData || !histData.entries || histData.entries.length < 2) {
      ...
      if (wrap) {
        wrap.innerHTML =
          '<div class="nt-chart-empty">Insufficient history data for timeline visualisation.<br>Data accumulates daily via the automated pipeline.</div>';
      }
      return;
    }
      ...
      for (var h = 0; h < hypIds.length; h++) {
        ...
        if (entry.hypotheses) {
          for (var j = 0; j < entry.hypotheses.length; j++) {
            if (entry.hypotheses[j].id === hid) {
              found = entry.hypotheses[j];
              break;
            }
          }
        }
      ...
      if (history[i].overcorrection_active) {
        ...
        if (ocParts.length === 3) {
          ...
          if (ocIdx >= 0) {
            flipMarkers.push({
              idx: ocIdx,
              color: NT_COLORS.overcorrection,
              dash: [2, 2],
              width: 2,
            });
          }
...
export function renderSignalBars(data) {
  ...
  if (ta) {
    ...
    var regimeCls = isCritical
      ? "critical"
      : isPositive
        ? "positive"
        : "neutral";
    var badgeLabel = isCritical
      ? "Critical"
      : isPositive
        ? "Positive"
        : regime.split(/[\s\u2014\u2013-]/)[0] || "Neutral";
    ...
    rows +=
      ...
      '<div class="sb-items">' +
      (currentP
        ? '<span class="sb-item">A$' +
          currentP.toFixed(2) +
          '</span><span class="sb-sep">|</span>'
        : "") +
      (dailyChange
        ? '<span class="sb-item ' + dailyCls + '">' + dailyChange + "</span>"
        : "") +
      (fromPeak
        ? '<span class="sb-sep">|</span><span class="sb-item ' +
          fromPeakCls +
          '">' +
          fromPeak +
          "</span>"
  ...
  if (tls) {
    var macSig = tls.macro_signal || 0;
    var macCls =
      macSig > 10 ? "positive" : macSig < -10 ? "downside" : "neutral";
    var macLabel =
      macSig > 10 ? "SUPPORTIVE" : macSig < -10 ? "HEADWIND" : "NEUTRAL";
    rows +=
      '<div class="sb-row">' +
      '<span class="sb-indicator ' +
      macCls +
      ...
      "</span>" +
      '<div class="sb-items">' +
      '<span class="sb-item">Signal ' +
      (macSig > 0 ? "+" : "") +
      macSig +
      "</span>" +
      (tls.external_weight
        ? '<span class="sb-sep">|</span><span class="sb-item">Weight ' +
          tls.external_weight +
          "%</span>"
        : "") +
      "</div>" +
  ...
  if (tls) {
    var secSig = tls.sector_signal || 0;
    var secCls =
      secSig > 10 ? "positive" : secSig < -10 ? "downside" : "neutral";
    var secLabel =
      secSig > 10 ? "POSITIVE" : secSig < -10 ? "NEGATIVE" : "NEUTRAL";
    var secName =
      (data.sector || "") + (data.sectorSub ? " / " + data.sectorSub : "");
  ...
  var skew = data._skew || computeSkewScore(data);
  var compCls =
    skew.score < -5 ? "downside" : skew.score > 5 ? "upside" : "neutral";
  var compBadge =
    skew.score < -5 ? "DOWNSIDE" : skew.score > 5 ? "UPSIDE" : "NEUTRAL";
  var scoreLbl = (skew.score > 0 ? "+" : "") + skew.score;
  var hyps =
    tls && tls.company_detail && tls.company_detail.hypotheses
  ...
  for (var di = 0; di < hyps.length; di++) {
    var dh = hyps[di],
      dw = dh.weight || 0;
    if (
      (skew.score < 0 && dh.direction === "downside" && dw > domMax) ||
  ...
  var sorted = hyps.slice().sort(function (a, b) {
  ...
  for (var ci = 0; ci < sorted.length; ci++) {
    ...
    var keyWord =
      (descParts[0] || "").toLowerCase() === "structural" && descParts[1]
        ? descParts[1]
        : descParts[0] || "";
    chipsHtml +=
      '<span class="sb-hyp-chip ' +
      chipCls +
      '">' +
      nCode +
      ...
      ? '<div class="sb-desc">' +
        ...
        (dominant
          ? " &bull; Dominant: " +
            dominant.title +
            " (" +
            dominant.direction.toUpperCase() +
            ")"
          : "") +
        "</div>"
      : "") +
...
export function renderPriceDrivers(data) {
  ...
  // Executive summary and full note from the report object
  var report = pd.report || {};
  var title = report.title || "What Drove the Price";
  var summary = report.executive_summary || "";
  var fullNote = report.full_note || "";

  // Driver stack badges
  var driverStackHtml = "";
  var stack = pd.driver_stack || {};
  var categories = [
    ["primary", "Primary"],
    ["secondary", "Secondary"],
  ...
  for (var c = 0; c < categories.length; c++) {
    var drivers = stack[categories[c][0]] || [];
    if (drivers.length > 0) {
      driverStackHtml += '<div class="pd-category">';
      driverStackHtml +=
        '<span class="pd-cat-label">' + categories[c][1] + "</span>";
      for (var d = 0; d < drivers.length; d++) {
        driverStackHtml +=
          '<span class="pd-driver-badge pd-' +
          categories[c][0] +
          '">' +
          drivers[d] +
          "</span>";
  ...
  var primaryDriver = pd.primary_driver || "";

  var body =
    '<div class="pd-primary-callout">' +
    '<span class="pd-conf-badge ' +
    confClass +
    '">' +
    conf.replace("_", " ") +
    " confidence</span> " +
  ...
  if (driverStackHtml) {
    body += '<div class="pd-driver-stack">' + driverStackHtml + "</div>";
  }

  if (summary) {
    body += '<div class="pd-summary">' + summary + "</div>";
  }

  if (fullNote) {
    body += '<div class="pd-full-note">' + fullNote + "</div>";
  }

  // Evidence quality
  var eq = pd.evidence_quality || {};
  if (eq.primary_evidence || eq.key_gap) {
    body += '<div class="pd-evidence-quality">';
    if (eq.primary_evidence) {
      body +=
        '<div class="pd-eq-item"><strong>Primary evidence:</strong> ' +
        eq.primary_evidence +
        "</div>";
    }
    if (eq.key_gap) {
      body +=
        '<div class="pd-eq-item"><strong>Key gap:</strong> ' +
        eq.key_gap +
        "</div>";
    }
    body += "</div>";
...
export function renderGoldSection(data) {
  ...
  if (ga.executive_summary) {
    body += '<div class="rs-subtitle">' + ga.executive_summary + "</div>";
  }

  // Investment view (bull / base / bear)
  var iv = ga.investment_view;
  if (iv) {
    body += '<div class="gold-scenarios">';
    body +=
      '<h4 class="rs-subtitle" style="margin-top:1.2em">Scenario Analysis</h4>';
    body +=
      '<table class="disc-table"><thead><tr><th>Scenario</th><th>View</th></tr></thead><tbody>';
    if (iv.bull_case)
      body +=
        '<tr><td><span class="sb-badge sb-pos">Bull</span></td><td>' +
        iv.bull_case +
        "</td></tr>";
    if (iv.base_case)
      body +=
        '<tr><td><span class="sb-badge sb-neut">Base</span></td><td>' +
        iv.base_case +
        "</td></tr>";
    if (iv.bear_case)
      body +=
        '<tr><td><span class="sb-badge sb-neg">Bear</span></td><td>' +
        iv.bear_case +
        "</td></tr>";
    body += "</tbody></table>";
    if (iv.monitoring_trigger) {
      body +=
        '<div class="tw-card" style="margin-top:0.8em"><div class="tw-title">Monitoring Trigger</div><div class="tw-body">' +
        iv.monitoring_trigger +
        "</div></div>";
    }
    body += "</div>";
  }
  ...
  if (km) {
    body += '<h4 class="rs-subtitle" style="margin-top:1.2em">Key Metrics</h4>';
    body +=
      '<table class="disc-table"><thead><tr><th>Metric</th><th>Value</th></tr></thead><tbody>';
    var metricLabels = {
      aisc_per_oz_usd: "AISC (US$/oz)",
      production_koz_annual: "Production (koz pa)",
      reserves_koz: "Reserves (koz)",
      resources_koz: "Resources (koz)",
      reserve_grade_gt: "Reserve Grade (g/t)",
      mine_life_years: "Mine Life (yrs)",
      net_cash_debt_usd_m: "Net Cash/Debt (US$m)",
      gold_price_assumption_usd_per_oz: "Gold Price Assumption (US$/oz)",
    };
    for (var mk in metricLabels) {
      if (km[mk] != null) {
        var val = typeof km[mk] === "number" ? km[mk].toLocaleString() : km[mk];
        body +=
          "<tr><td>" + metricLabels[mk] + "</td><td>" + val + "</td></tr>";
      }
    }
    body += "</tbody></table>";
  }
  ...
  if (assets && assets.length) {
    body +=
      '<h4 class="rs-subtitle" style="margin-top:1.2em">Asset Portfolio</h4>';
    body +=
      '<table class="disc-table"><thead><tr><th>Asset</th><th>Stage</th><th>Ownership</th><th>Production (koz)</th><th>AISC (US$/oz)</th></tr></thead><tbody>';
    for (var ai = 0; ai < assets.length; ai++) {
      ...
      body +=
        "<td>" +
        (a.ownership_pct != null ? a.ownership_pct + "%" : "-") +
        "</td>";
      body +=
        "<td>" +
        (a.annual_production_koz != null
          ? a.annual_production_koz.toLocaleString()
          : "-") +
        "</td>";
      body +=
        "<td>" +
        (a.aisc_per_oz_usd != null ? a.aisc_per_oz_usd.toLocaleString() : "-") +
        "</td>";
      body += "</tr>";
    }
    body += "</tbody></table>";
  ...
  if (sc) {
    body +=
      '<h4 class="rs-subtitle" style="margin-top:1.2em">Quality Scorecard</h4>';
    body +=
      '<table class="disc-table"><thead><tr><th>Dimension</th><th>Score</th></tr></thead><tbody>';
    var dims = {
    ...
    for (var dk in dims) {
      if (sc[dk] != null) {
        body += "<tr><td>" + dims[dk] + "</td><td>" + sc[dk] + "/100</td></tr>";
      }
    }
    if (sc.composite_score != null)
      body +=
        '<tr style="font-weight:600"><td>Composite</td><td>' +
        sc.composite_score +
        "/100</td></tr>";
    body += "</tbody></table>";
  }
  ...
  if (risks) {
    var riskSections = [
      ["top_failure_modes", "Failure Modes"],
      ["hard_risk_flags", "Hard Risk Flags"],
      ["technical_red_flags", "Technical Red Flags"],
      ["information_gaps", "Information Gaps"],
    ];
    for (var ri = 0; ri < riskSections.length; ri++) {
      var rItems = risks[riskSections[ri][0]];
      if (rItems && rItems.length) {
        body +=
          '<h4 class="rs-subtitle" style="margin-top:1em">' +
          riskSections[ri][1] +
          "</h4><ul>";
        for (var rj = 0; rj < rItems.length; rj++) {
          body += "<li>" + rItems[rj] + "</li>";
        }
        body += "</ul>";
      }
    }
    ...
    var recClass =
      rec.status === "Proceed"
        ...
        : rec.status === "Pass"
          ...
          : "sb-neut";
    body +=
      '<div style="margin-top:1.2em"><span class="sb-badge ' +
      recClass +
      '">' +
      rec.status +
      "</span>";
    if (rec.reason) body += " " + rec.reason;
    if (rec.confidence != null)
      body +=
        ' <span style="opacity:0.6">(confidence: ' +
  ...
  return (
    ...
    '-gold-analysis">' +
    '<div class="rs-header"><div class="rs-header-text">' +
    '<div class="rs-number">Gold</div>' +
    '<div class="rs-title">Gold Analysis</div>' +
    '</div><button class="rs-toggle" onclick="window.toggleSection(this)" aria-label="Toggle section">' +
    RS_CHEVRON +
    "</button></div>" +
    '<div class="rs-body">' +
...
// ============================================================
// ON-DEMAND PRICE DRIVER FETCH (used by batch-refresh.js)
// Separate from renderPriceDrivers() which renders embedded data
// ============================================================
...
export function renderPriceDriversContent(container, driverData) {
  ...
  var moveCls =
    movePct > 1 ? "pd-move-up" : movePct < -1 ? "pd-move-down" : "pd-move-flat";
  var confCls =
    conf === "very_high" || conf === "high"
      ? "pd-conf-high"
      : conf === "moderate"
        ? "pd-conf-mod"
        : "pd-conf-low";
  var dateStr = _formatDriverDate(
    driverData.analysis_date || meta.analysis_date,
  );
  ...
  var primary =
    driverData.primary_driver || rc.most_likely_primary_driver || "";
  if (!primary && ds.primary && ds.primary.length > 0) primary = ds.primary[0];
  if (!primary && rt.primary_driver_paragraph)
    primary = _truncate(rt.primary_driver_paragraph, MAX);
  if (primary) {
    bullets.push("<b>Primary driver:</b> " + _truncate(primary, MAX));
  }

  // Bullet 2: Secondary drivers
  var secondaries = ds.secondary || rc.secondary_drivers || [];
  if (secondaries.length > 0) {
    bullets.push("<b>Secondary:</b> " + _truncate(secondaries.join("; "), MAX));
  } else if (rt.secondary_drivers_paragraph) {
    bullets.push(
      "<b>Secondary:</b> " + _truncate(rt.secondary_drivers_paragraph, MAX),
    );
  }
  ...
  if (peerText && macroText) {
    bullets.push(
      "<b>Peer and macro context:</b> " +
        _truncate(peerText + ". " + macroText, MAX),
    );
  } else if (peerText) {
  ...
  // Bullet 4: Ruled out
  var rejected = ds.rejected || rc.rejected_explanations || [];
  if (rejected.length > 0) {
    bullets.push(
      "<b>Ruled out:</b> " + _truncate(rejected.slice(0, 3).join("; "), MAX),
    );
  } else if (rt.rejected_explanations_paragraph) {
    bullets.push(
      "<b>Ruled out:</b> " + _truncate(rt.rejected_explanations_paragraph, MAX),
    );
  }
